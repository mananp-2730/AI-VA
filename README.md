...
* **Global Database Identity Sync:** Upgraded the SQLAlchemy memory pipeline to permanently stamp every saved insight with the user's verified Google SSO email. Insights now dynamically sync across different devices and browser profiles, ensuring true cross-platform data continuity.
...
* **Native Vector PDF Engine (Executive Reporting):** Ripped out standard HTML-to-image screenshot tools and engineered a zero-latency, client-side native compilation engine using `jsPDF`. AI-VA dynamically draws razor-sharp vector typography, extracts isolated Chart.js canvases, and compiles a branded, boardroom-ready document entirely in the browser's memory without incurring backend compute costs.
...
* **The "Zero-Latency" Illusion (WebSocket Streaming):** Upgraded the core HTTP orchestration pipeline to a persistent `ws://` WebSocket tunnel. AI-VA utilizes `generate_content_stream` to fire text tokens to the UI the millisecond they are generated, while silently buffering the complex Chart.js JSON configuration in the background. This effectively drops perceived Time-to-Insight (TTI) to zero.
* **Triple-Net Architecture (Graceful Failure):** Built an invincible error-handling pipeline. Net 1 utilizes an AI Guardrail prompt to proactively refuse irrelevant non-business queries. Net 2 features a Python exception catcher that intercepts broken SQL generation or database locks. Net 3 gracefully translates these intercepts into polite, conversational UI responses, completely eliminating raw 500 Server Errors from the user experience.
* **Voice-Activated UI Manipulation (Zero-Backend Filtering):** Engineered a lightweight, client-side NLP interceptor within the JavaScript frontend. When a user issues a purely visual command (e.g., "Change this to a pie chart"), the frontend intercepts the transcript, aborts the HTTP/WebSocket request to save LLM token costs, and mutates the active Chart.js configuration locally in real-time.
...
* **Decision 10: WebSocket Streaming vs. Standard HTTP POST**
  * **The Trade-off:** We increased backend architectural complexity (managing `ws://` connections and parsing stream chunks) instead of relying on a standard, simple HTTP `fetch()` request.
